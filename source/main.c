...
int main() {
    *(unsigned int*)0x04000000 = 0x0403; // Set GBA rendering context to MODE 3 Bitmap Rendering
    ...
    float angle = 0.0f;
    float angularSpeed = 0.05f; // Adjust speed for desired animation
    unsigned short colorIncrement = 0x001; // Increment for color transition (adjust for slower transition)

    while (1) {
        // Fill screen buffer with white color
        for (int i = 0; i < SCREEN_WIDTH * SCREEN_HEIGHT; i++) {
            SCREEN_BUFFER[i] = 0xFFFF; // Fill screen buffer with white color
        }
